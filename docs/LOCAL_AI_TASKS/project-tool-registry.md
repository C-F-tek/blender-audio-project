...
| `Tools/ai/runtime_universe/full_toolbox_telemetry_summary/cli.py` | `build_full_toolbox_run_telemetry_summary` | `FULL_RUN_EVIDENCE` | stable | candidate if report-only | run telemetry summary | no | no | `output/analysis` or requested output | `docs/LOCAL_VALIDATION_EVIDENCE/full_toolbox_run_telemetry_summary_*.json/.md` | `python -m py_compile .\Tools\ai\runtime_universe\full_toolbox_telemetry_summary\cli.py` | report-only | Reads GPU/NPU sync report; does not compute timings itself. |
