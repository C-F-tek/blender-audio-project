...
| `Tools/ai/shared_toolbox_bundle/cli.py` | `build_shared_toolbox_ai_to_ai_bundle` | `FULL_RUN_EVIDENCE;PATCH_PLAN_SUPPORT` | stable | candidate if report-only | AI-to-AI production bundle | no | no | `output/analysis/shared_toolbox_ai_to_ai_final_summary_*.json` | `docs/LOCAL_VALIDATION_EVIDENCE/shared_toolbox_ai_to_ai_bundle_*.json/.md` | `python -m py_compile .\Tools\ai\shared_toolbox_bundle\cli.py` | compact only; no raw output dump | Must expose provider advisory degradation. |
| `Tools/ai/full_toolbox_telemetry_summary/cli.py` | `build_full_toolbox_run_telemetry_summary` | `FULL_RUN_EVIDENCE` | stable | candidate if report-only | run telemetry summary | no | no | `output/analysis` or requested output | `docs/LOCAL_VALIDATION_EVIDENCE/full_toolbox_run_telemetry_summary_*.json/.md` | `python -m py_compile .\Tools\ai\full_toolbox_telemetry_summary\cli.py` | report-only | Reads GPU/NPU sync report; does not compute timings itself. |
