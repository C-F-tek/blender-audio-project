...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| `Tools/ai/runtime_tool/agent_broker.py` | `agent_runtime_tool_broker` | `BROKER_TOOL;SUPPORT_LIBRARY` | stable dispatcher | n/a dispatcher | broker runtime lane | no | no | caller-defined JSON/MD reports | none directly | `python -m py_compile .\Tools\ai\runtime_tool\agent_broker.py` | allowlist-only; no arbitrary shell | Keep as broker gate, not as generic executor. |
...
| `Tools/ai/agent_context/memory_inventory/cli.py` | `build_agent_memory_inventory` | `BROKER_TOOL;MEMORY_LANE;FULL_RUN_EVIDENCE` | stable | yes | memory inventory | no | no by default | `output/ai_pipeline/*memory_inventory*.json` | compact evidence only | `python -m py_compile .\Tools\ai\agent_context\memory_inventory\cli.py` | read-only unless explicit policy | Do not enable persistent writes by default. |
| `Tools/ai/agent_context/agnostic_tool_inventory/cli.py` | `build_agent_agnostic_tool_inventory` | `BROKER_TOOL;FULL_RUN_EVIDENCE` | stable | yes | tool inventory | no | no | `output/ai_pipeline/*tool_inventory*.json` | compact evidence only | `python -m py_compile .\Tools\ai\agent_context\agnostic_tool_inventory\cli.py` | report-only | Keep feeding tool-placement audits. |
| `Tools/ai/agent_context/transient_request_context/cli.py` | `build_agent_transient_request_context` | `BROKER_TOOL;PATCH_PLAN_SUPPORT` | stable | yes | transient request context | no | no | `output/ai_pipeline/*transient_request_context*.json` | compact evidence only | `python -m py_compile .\Tools\ai\agent_context\transient_request_context\cli.py` | no source writes | Useful as task-context lane. |
...
| `Tools/ai/code_product/interpreter_report/cli.py` | `build_code_interpreter_report` | `BROKER_TOOL;FULL_RUN_EVIDENCE` | stable | yes | static code-interpreter report | no | no | `output/ai_pipeline/*code_interpreter*.json` | compact evidence only | `python -m py_compile .\Tools\ai\code_product\interpreter_report\cli.py` | report-only | Do not execute arbitrary code. |
...
| `python -m Tools.ai build_refactor_duplication_audit` | `build_refactor_duplication_audit` | `BROKER_TOOL;PATCH_PLAN_SUPPORT` | stable | yes | refactor evidence | no | no | `output/ai_pipeline/*duplication_audit*.json` | compact evidence only | `python -m py_compile .\Tools\ai\repository_product\refactor_duplication_audit\cli.py` | report-only | Feed patch plans, do not apply them. |

## Full-run and evidence tools

| path | tool_id | classification | current_status | broker_eligible | full_run_lane | provider_execution | source_writes | runtime_outputs | git_tracked_outputs | validation_command | guardrails | promotion_notes |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| `Tools/ai/runtime_tool/usage_evidence/cli.py` | `build_runtime_tool_capability_manifest` | `FULL_RUN_EVIDENCE` | stable | candidate if report-only | runtime tool evidence | no | no | `output/validation/runtime_tool_capability_manifest_*.json` | `docs/LOCAL_VALIDATION_EVIDENCE/runtime_tool_capability_manifest_*.json/.md` | `python -m py_compile .\Tools\ai\runtime_tool\usage_evidence\cli.py` | preserve broker reports; no output commit | Required for broker evidence completeness. |
| `Tools/ai/agent_context/shared_toolbox_bundle/cli.py` | `build_shared_toolbox_ai_to_ai_bundle` | `FULL_RUN_EVIDENCE;PATCH_PLAN_SUPPORT` | stable | candidate if report-only | AI-to-AI production bundle | no | no | `output/analysis/shared_toolbox_ai_to_ai_final_summary_*.json` | `docs/LOCAL_VALIDATION_EVIDENCE/shared_toolbox_ai_to_ai_bundle_*.json/.md` | `python -m py_compile .\Tools\ai\agent_context\shared_toolbox_bundle\cli.py` | compact only; no raw output dump | Must expose provider advisory degradation. |
| `Tools/ai/provider_mesh/gpu_npu_run_sync_analysis/cli.py` | `analyze_gpu_npu_run_sync` | `PROVIDER_DIAGNOSTIC;FULL_RUN_EVIDENCE` | stable | candidate if report-only | GPU/NPU sync diagnostics | no | no | `output/validation/*gpu_npu*sync*.json/.md` | compact evidence only | `python -m py_compile .\Tools\ai\provider_mesh\gpu_npu_run_sync_analysis\cli.py` | must prefer `rounds[*].elapsed_seconds` | Primary source for GPU round timing metrics. |
| `Tools/ai/repository_product/github_evidence_bundle.py` | `build_github_evidence_bundle` | `FULL_RUN_EVIDENCE;DOCS_MAINTENANCE` | stable | no by default | evidence bundle builder | no | writes evidence docs | `docs/LOCAL_VALIDATION_EVIDENCE/*` | compact bundle JSON/MD | `python -m py_compile .\Tools\ai\repository_product\github_evidence_bundle.py` | never copy raw heavy output | Manual/evidence stage only. |
...
| path | tool_id | classification | current_status | broker_eligible | full_run_lane | provider_execution | source_writes | runtime_outputs | git_tracked_outputs | validation_command | guardrails | promotion_notes |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| `Tools/ai/provider_mesh/local_provider_probe.py` | `run_local_provider_probe` | `PROVIDER_DIAGNOSTIC` | stable | no by default | provider probe | yes when requested | no | `output/validation/local_provider_probe.json/.md` | compact evidence only | `python -m py_compile .\Tools\ai\provider_mesh\local_provider_probe.py` | explicit provider diagnostics only | Safe diagnostic, but not default broker. |
...
| path | tool_id | classification | current_status | broker_eligible | full_run_lane | provider_execution | source_writes | runtime_outputs | git_tracked_outputs | validation_command | guardrails | promotion_notes |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| `Tools/ai/deterministic_recommendations/cli.py` | `build_deterministic_recommendations` | `PATCH_PLAN_SUPPORT;FULL_RUN_EVIDENCE` | stable | no by default | deterministic recommendations | no | no | `output/patch_specs` or requested report | compact evidence only | `python -m py_compile .\Tools\ai\deterministic_recommendations\cli.py` | forbidden target prefixes enforced | Keep anti-evidence target hygiene. |
| `Tools/ai/agent_review/patch_plan/cli.py` | `agent_review_patch_plan` | `PATCH_PLAN_SUPPORT` | stable | no by default | patch-plan generation | no | no | `output/patch_specs/agent_review_patch_plan.json/.md` | compact evidence only | `python -m py_compile .\Tools\ai\agent_review\patch_plan\cli.py` | review-only; no apply | Canonical patch-plan builder. |
| `Tools/ai/patch_product/patch_notes_quality_product/cli.py` | `build_patch_notes_quality_product` | `PATCH_PLAN_SUPPORT;FULL_RUN_EVIDENCE` | active product builder | no by default | patch notes quality product | no | no | `docs/LOCAL_VALIDATION_EVIDENCE/patch_notes_quality_product_*.json/.md`; operational FTS under `output/**` | compact evidence JSON/MD | `python -m Tools.validation patch_notes_quality_product_smoke --repo-root .` | report-only; manual review required; no provider call by builder | Reuses patch-plan quality IO/FTS/scoring surfaces and consumes runtime evidence/capability evidence. |
| `Tools/ai/agent_review/code_patch_plan_cli.py` | `agent_review_code_patch_plan` | `PATCH_PLAN_SUPPORT` | stable | no | code patch-plan lane | no | no | `output/patch_specs/*code_patch_plan*.json/.md` | compact evidence only | `python -m py_compile .\Tools\ai\agent_review\code_patch_plan_cli.py` | no automatic apply | Manual review required. |
| `Tools/ai/code_product/edit_proposal_from_plan/cli.py` | `build_code_edit_proposal_from_plan` | `PATCH_PLAN_SUPPORT` | stable | no | code edit proposal | no | no | `output/patch_specs/*code_edit_proposal*.json` | compact evidence only | `python -m py_compile .\Tools\ai\code_product\edit_proposal_from_plan\cli.py` | proposal-only | Never apply directly. |
| `Tools/ai/code_product/patch_artifact_pack/cli.py` | `build_code_patch_artifact_pack` | `PATCH_PLAN_SUPPORT;FULL_RUN_EVIDENCE` | stable | no | artifact pack | no | writes artifact pack | output artifact pack | compact evidence only | `python -m py_compile .\Tools\ai\code_product\patch_artifact_pack\cli.py` | no source writes unless explicit | Keep packs out of Git unless policy allows. |
...
| `Tools/workflow/workflow_run/ai_runtime_diagnostics.py` | `ai_runtime_diagnostics` | `PROVIDER_DIAGNOSTIC;PROJECT_TOOL` | candidate | no | runtime diagnostics | optional | no | `output/validation/**` | compact evidence only | `python -m py_compile .\Tools\workflow\workflow_run\ai_runtime_diagnostics.py` | diagnostic only | Good preflight candidate. |
...
| `Tools/ai/code_product/interpreter_report/*.py` | `code_interpreter_report_modules` | `SUPPORT_LIBRARY;FULL_RUN_EVIDENCE` | internal | no | static code report internals | no | no | caller-defined | none directly | `python -m py_compile .\Tools\ai\code_product\interpreter_report\*.py` | helper-only | Implementation for `python -m Tools.ai build_code_interpreter_report`. |
| `Tools/ai/repository_product/repository_consistency_map/*.py` | `repository_consistency_map_modules` | `SUPPORT_LIBRARY;FULL_RUN_EVIDENCE` | internal | no | repository consistency internals | no | no | caller-defined | none directly | `python -m py_compile .\Tools\ai\repository_product\repository_consistency_map\*.py` | helper-only | Implementation split for `python -m Tools.ai build_repository_consistency_map`. |
