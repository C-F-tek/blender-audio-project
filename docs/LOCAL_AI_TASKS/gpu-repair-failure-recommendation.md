...
```text
ia_carmine/providers/provider_mesh/gpu_repair_failure_recommendation.py
```
...
recommended_next_layer: python -m ia_carmine.cli agent_review_patch_plan
```
...
local parser not reusing the shared ia_carmine._shared.model_json helper everywhere
...
```text
reuse ia_carmine._shared.model_json.parse_model_json_object in GPU planner parsing paths
