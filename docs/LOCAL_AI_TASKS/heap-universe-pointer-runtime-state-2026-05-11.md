...
Il file `ia_carmine/runtime/run/profiles/heap_runtime_launcher_profiles.json` espone solo questi profili:
...
- materializza CLI args supportati da `python -m ia_carmine.cli heap_context_closure`;
...
- genera `postrun_package_command` per `python -m ia_carmine.cli external_heap_postrun_package`;
...
- target corretto per `python -m ia_carmine.cli heap_context_closure`;
- target corretto per `python -m ia_carmine.cli external_heap_postrun_package`.
...
Per testare l'universo completo non basta stampare il command builder. Serve eseguire direttamente `python -m ia_carmine.cli heap_context_closure` o copiare/eseguire il comando prodotto dal builder.
...
La sequenza post-run e' ora automatizzabile con `python -m ia_carmine.cli external_heap_postrun_package` e il command builder genera `postrun_package_command`, ma `python -m ia_carmine.cli heap_context_closure` non lo invoca automaticamente.
...
& $RepoPy -m py_compile `
  .\ia_carmine\runtime\provider_runtime_blackboard\cli.py `
  .\ia_carmine\runtime\heap_runtime\launcher_command\cli.py `
  .\ia_carmine\python -m ia_carmine.cli normalize_heap_final_causality `
  .\ia_carmine\runtime\external_heap\block_pointer_manifest.py `
  .\ia_carmine\python -m ia_carmine.cli compose_external_heap_block_response `
