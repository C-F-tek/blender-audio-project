...
1. Read the latest `unified_local_ai_refactor_manifest.json`.
2. Inspect provider diagnostics and workload quality reports.
3. Inspect decision loop recommendation and patch-plan counts.
4. Group evidence, patch plan, telemetry, capability manifest, full toolbox telemetry summary and AI-to-AI bundle/final summary.
5. Push runtime artifact bundle as a draft GitHub release asset or attach to PR; do not commit `output/**`.
6. Select a controlled patch subset manually.
7. Apply source/docs changes in a normal reviewed commit only after focused validation.
