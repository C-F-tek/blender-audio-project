...
| `full0to10_product_pr_chain_smoke*.json` | `Tools/validation/run_full0to10_product_pr_chain_smoke.py` | self-report plus report-contract validation | `schema_version`, `kind=unified_run_product_pr_chain_smoke`, `legacy_smoke_name`, `passed`, `commands`, `workflow_trace`, `errors`, `warnings` | Legacy filename for a unified-run smoke. Verifies task report -> deterministic apply report -> auto-include -> review PR prepare product -> final unified chain contract. |
