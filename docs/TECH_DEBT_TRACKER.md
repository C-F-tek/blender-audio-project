...
Technical debt that affects manifest visibility, runtime telemetry, capability manifests, full toolbox telemetry summary, shared AI-to-AI bundles, discovery/index/CSV surfaces, oversized Markdown entrypoints, product PR workflow, patch suggestion separation or measurable capability limits should be tracked here when not fixed immediately.
...
| TD-019 | Full-context golden proposal promotion | P2 | in_progress | The deterministic full-context golden proposal generator emits P1-P6 proposal families, and later tooling builds agent-review patch plans and richer evidence bundles. | Valuable proposal output can remain inert or be applied too broadly if promotion is not staged. | Promote one proposal or patch-plan family at a time through reviewed patch specs or patch-suggestion product flow, focused validators, compact evidence and companion telemetry/capability summaries. | 2026-05-08 |
...
| TD-030 | Markdown-to-review-PR include paths | P1 | open | The product path still requires explicit `ReviewPrIncludePath` / `prepare_review_pr.py --include-path`. | A task Markdown can produce valid deterministic operations, but the review PR step can still need manual path selection and can drift from apply reports. | Add reviewed include-path autodiscovery from `patch_suggestion_bundle_apply` reports, with allowlist/path-policy validation and visible review-PR evidence. | 2026-05-08 |
| TD-031 | Draft PR creation | P2 | open | `prepare_review_pr.py` does not create GitHub PRs as draft yet. | The desired product may need manual PR state management or normal PRs before human review. | Add explicit draft PR support only behind a flag, preserving no-PR/no-push defaults and reporting `draft=true` in review PR evidence. | 2026-05-08 |
...
| ID | Area | Priority | Status | Symptom | Risk | Recommended action | Last reviewed |
|---|---|---:|---|---|---|---|---|
| TD-033 | area | P2 | open | symptom | risk | action | YYYY-MM-DD |
