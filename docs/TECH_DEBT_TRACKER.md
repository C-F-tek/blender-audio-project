...
| TD-006 | Formal JSON schemas | P2 | in_progress | JSON contracts are documented but not fully enforced. | AI artifacts can drift. | AI pipeline matrix, per-case reports, GitHub evidence bundles, repository proposal reports and proposal-derived patch-spec drafts now have focused contract docs/validators; continue with provider probe raw reports, scene specs and music summaries after report contracts remain stable. | 2026-05-01 |
