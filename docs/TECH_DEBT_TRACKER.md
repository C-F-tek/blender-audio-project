...
| TD-004 | Ready To Jazz package | P2 | open | Large standalone/monolithic script. | Harder patching and reuse. | Do not split yet; extract shared infrastructure first and migrate one call site at a time after workstation validation. | 2026-04-30 |
...
| TD-006 | Formal JSON schemas | P2 | open | JSON contracts are documented but not fully enforced. | AI artifacts can drift. | Add schema validators gradually for AI pipeline reports, scene specs and music summaries; begin with gap tables before strict checks. | 2026-04-30 |
| TD-007 | NPU pipeline decomposition | P2 | open | Some NPU orchestration files remain large. | Harder testing and provider replacement. | Plan split into config, context builder, prompts, providers, validators, artifact writer and runner; do not split from GitHub-only access. | 2026-04-30 |
