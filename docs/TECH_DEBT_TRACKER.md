...
| TD-007 | NPU pipeline decomposition | P2 | in_progress | Some NPU orchestration files remain large. | Harder testing and provider replacement. | Helper package work is active under `Tools/npu/pipeline/`; IO, path/contract, prompt-payload, context-summary and support-file write-planning groups are being migrated incrementally before provider phases. | 2026-04-30 |
