...
| TD-007 | NPU pipeline decomposition | P2 | in_progress | Some NPU orchestration files remain large. | Harder testing and provider replacement. | Helper package work is active under `Tools/npu/pipeline/`; IO, path/contract and prompt-payload wiring are being migrated one helper group at a time before context builder, artifact writer and provider phases. | 2026-04-30 |
...
| TD-016 | NPU runtime wiring | P2 | in_progress | `Tools/npu/run_dual_ai_pipeline.py` still owns orchestration while helper groups are being adopted incrementally. | Partial migration can drift if helper/runtime equivalence is not validated after each group. | Continue one helper group at a time with focused NPU helper validation, full local validation and regenerated indexes; keep provider adapters last. | 2026-04-30 |
