...
|---|---|---|
| `Tools/workflow/audio_analysis/` and `Tools/workflow/scene_spec/` | Audio analysis, summary building, scene-spec normalization | Keep importable services plus thin CLI wrappers. |
...
| `Tools/npu/_shared/ollama_runtime.py` | model runtime provider | Provider execution adapter phase, not current default. |
...
| `Tools/workflow/scene_spec/` | scene spec model and validators | Keep under workflow service modules unless promoted to a shared library later. |
