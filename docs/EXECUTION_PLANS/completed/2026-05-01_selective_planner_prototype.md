...
Implemented and merged through PR #55:

```text
feat(ai): add report-only selective planner prototype (#55)
```

Merge commit:

```text
f88c8453e0945bcb117372a3aa62e597010f030f
```

The milestone includes:

- report-only selective planner;
- dedicated selective execution plan validator;
- stable selective planner documentation;
- validation README registration;
- docs index update;
- compact real GPU/NPU evidence for this milestone.
...
selective_execution_plan_validation.passed: true
```

Provider posture remains unchanged:

```text
Ollama/GPU -> primary advisory behind quality gate
...
| `docs/EXECUTION_PLANS/completed/2026-05-01_selective_planner_prototype.md` | move/update | Durable completed task record. |
...
1. Add validator ranking/scoring in a future report-only iteration.
2. Add a generator that converts `recommended_patch_specs` into draft patch specs without replacements.
3. Define formal provider evidence freshness and promotion gates before changing provider behavior.
