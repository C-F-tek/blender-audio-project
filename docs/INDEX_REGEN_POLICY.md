...
If the app also regenerates files under `output/`, use:
...
```powershell
powershell.exe -ExecutionPolicy Bypass -File .\Tools\git\auto_push_generated_data.ps1 -DryRun
```
...
Indexes must be rebuilt by the project app/local maintainer workflow whenever structural or workflow-relevant changes are made. After regeneration, the app may call `Tools/git/auto_push_generated_data.ps1` to push generated technical data automatically.
