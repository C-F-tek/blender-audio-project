...
# Original content moved to part-001_sections to keep maintained scripts below the line budget.
$__IaCarminePartRoot = if ($PSScriptRoot) { $PSScriptRoot } else { Split-Path -Parent $MyInvocation.MyCommand.Path }
. (Join-Path $__IaCarminePartRoot "part-001_sections/section-001.ps1")
. (Join-Path $__IaCarminePartRoot "part-001_sections/section-002.ps1")
