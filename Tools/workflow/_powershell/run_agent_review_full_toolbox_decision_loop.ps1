...
$PythonPackage = Join-Path $WorkflowRoot "run_agent_review_full_toolbox_decision_loop"
if (-not (Test-Path -LiteralPath $PythonPackage -PathType Container)) {
    throw "Full toolbox decision-loop Python package not found: $PythonPackage"
}
...
$ForwardArgs = @("-m", "Tools.workflow", "run_agent_review_full_toolbox_decision_loop")
