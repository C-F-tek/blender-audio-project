...
try {
    ...
    Invoke-Step -Name "ai pipeline dry-run matrix" -Command "python" -Arguments @("-m", "Tools.ai", "run_pipeline_dry_run_matrix", "--repo-root", ".", "--continue-on-error", "--matrix-workers", $MatrixWorkers.ToString(), "--repeat-cases", $RepeatCases.ToString())
