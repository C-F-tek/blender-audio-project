...
def build_fixtures(repo_root: Path, stamp: str) -> dict[str, str]:
    ...
    write_fixture(paths["full_toolbox_telemetry"], {**common, "kind": "full_toolbox_run_telemetry_summary", "provider_execution_performed": True, "provider_evidence": {"provider_execution_performed": True, "gpu_provider_execution_performed": True, "gpu0_peer_support_provider_execution_performed": True, "npu_micro_tool_lane_performed": True}})
...
def main() -> int:
    ...
    report = {
        ...
        "manual_review_required": True,
        "positive": {"passed": positive.get("passed"), "quality_gate_passed": positive.get("quality_gate_passed"), "classification": positive.get("classification"), "patch_note_count": len(positive.get("patch_notes", []))},
        "negative": {"passed": negative.get("passed"), "quality_gate_passed": negative.get("quality_gate_passed"), "classification": negative.get("classification"), "fallback_path_note_count": len(negative.get("fallback_path_notes", []))},
