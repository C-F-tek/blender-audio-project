...
def evidence(repo_root: Path, name: str, path: Path) -> dict[str, Any]:
    ...
    for key in ("task_count", "response_count", "tool_request_count", "tool_execution_count", "runtime_tool_execution_count"):
        if key in data:
...
def build_report(args: argparse.Namespace) -> dict[str, Any]:
    ...
    tool_count = len(tool_requests.get("tool_requests", [])) if isinstance(tool_requests.get("tool_requests"), list) else 0
    if tool_count <= 0:
    ...
    if exchange.get("source_writes_performed") is True or response.get("source_writes_performed") is True:
        errors.append("source_writes_performed_in_peer_exchange")
        add(classifications, "peer_exchange_guardrail_violation")
    for item in response.get("classifications", []) if isinstance(response.get("classifications"), list) else []:
...
def main() -> int:
    ...
    parser.add_argument("--primary-advisory", default="output/validation/gpu1_primary_advisory_{stamp}.json")
    parser.add_argument("--task-packet", default="output/validation/gpu0_peer_task_packet_{stamp}.json")
    ...
    parser.add_argument("--output", default="output/validation/ai_peer_exchange_contract_{stamp}.json")
    parser.add_argument("--markdown-output", default="output/validation/ai_peer_exchange_contract_{stamp}.md")
