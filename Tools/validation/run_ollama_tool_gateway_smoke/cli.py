...
from Tools.ai.ollama_tool_gateway import common as gateway_common
...
def build_report(repo_root: Path) -> dict[str, Any]:
    errors: list[str] = []
    warnings: list[str] = []
    gateway_path = repo_root / "Tools" / "ai" / "ollama_tool_gateway"
    compile_targets = [
        gateway_path / "__init__.py",
        gateway_path / "cli.py",
        gateway_path / "common.py",
    ]
    ...
    for compile_target in compile_targets:
        try:
            ...
            errors.append(f"py_compile failed for {compile_target.name}: {exc}")
    ...
    if not errors:
        ...
        for sample_path, expected_error in samples.items():
            actual = gateway_common.path_policy_error(sample_path)
