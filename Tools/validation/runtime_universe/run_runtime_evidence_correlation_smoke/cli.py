...
def seed_fixture(repo: Path) -> dict[str, Path]:
    paths = {
        ...
        "heap_entry": repo
        / f"output/local_ai_runs/{STAMP}_unified/ai_packets/heap_exchange_runtime_entry.json",
        "heap_peer_runtime": repo
        / f"output/local_ai_runs/{STAMP}_unified/ai_packets/heap_peer_runtime_manifest.json",
        ...
        "review_pr_prepare": repo
        / f"output/validation/review_pr_prepare_real_product_{STAMP}.json",
    ...
    write_json(
        paths["preflight"],
        fixture_report(
            "real_product_preflight_gate",
            steps=["full_product_pr_chain"],
            full_product_pr_chain=True,
        ),
    )
    write_json(
        paths["heap_entry"],
        fixture_report(
            "heap_exchange_runtime_entry", task_ingress=True, heap_exchange=True, runtime_entry=True
        ),
    )
    write_json(
        paths["heap_peer_runtime"],
        fixture_report(
            "heap_peer_runtime_manifest",
            peers=["GPU1 primary advisory", "GPU0 OpenVINO peer", "NPU micro peer"],
        ),
    )
    write_json(
        paths["gpu0_peer"],
        fixture_report("openvino_gpu0_workload", gpu0=True, openvino=True, peer=True),
    )
    write_json(
        paths["npu_micro_peer"], fixture_report("npu_micro_peer", npu=True, micro=True, peer=True)
    )
    write_json(
        paths["shared_memory"],
        fixture_report(
            "shared_toolbox_ai_to_ai_bundle", shared=True, memory=True, heap=True, ai_to_ai=True
        ),
    )
    write_json(
        paths["tool_broker"],
    ...
    write_json(
        ...
        fixture_report(
            ...
            closure_state="ready_for_final_chain_contract",
            closure=True,
            audit=True,
            heap_exchange=True,
        ),
    ...
    write_json(
        paths["product_readiness"],
        fixture_report("review_pr_product_readiness", product=True, ready=True),
    )
    write_json(
        paths["review_pr_prepare"],
        fixture_report("review_pr_prepare", product_commit=True, git_commit_performed=True),
    )
    write_json(
        paths["unified_chain_contract"],
        fixture_report("unified_chain_contract", edges=[{"edge": "heap", "passed": True}]),
    )
...
def run_validator(
    source_repo: Path, fixture_repo: Path, output: Path, markdown: Path
    ...
    return subprocess.run(
        [
            sys.executable,
            str(source_repo / "Tools/validation/runtime_universe/check_runtime_evidence_correlation/cli.py"),
            "--repo-root",
            str(fixture_repo),
            "--stamp",
            STAMP,
...
def main() -> int:
    ...
    parser.add_argument(
        "--output", default="output/validation/runtime_evidence_correlation_smoke.json"
    )
    ...
    with tempfile.TemporaryDirectory(prefix="runtime-evidence-correlation-smoke-") as tmp:
        ...
        report = (
            json.loads(validator_output.read_text(encoding="utf-8-sig"))
    ...
    require(
        report.get("passed") is True, errors, "runtime evidence correlation report did not pass"
    )
    require(
        not report.get("missing_surfaces"),
        errors,
        "runtime evidence correlation reported missing surfaces",
    )
    require(
        not report.get("failed_surfaces"),
        errors,
        "runtime evidence correlation reported failed surfaces",
    )
    require(
        len(report.get("surfaces") or []) == 11,
        errors,
        "runtime evidence correlation surface count mismatch",
    )
