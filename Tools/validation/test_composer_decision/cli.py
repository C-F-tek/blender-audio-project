...
def test_all_rejected_gives_diagnostic_only() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="composer-decision-") as tmp:
        ...
        text = "TARGET_FILES: Tools/ai/compose_heap_final_proposals/cli.py\n\ndef foo():\n    return 1"
        write_proposal(proposal_dir, "001", "Tools/ai/compose_heap_final_proposals/cli.py", text, False)
        write_proposal(proposal_dir, "002", "Tools/ai/compose_heap_final_proposals/cli.py", text, False)
        payload = run_composer(run_dir, ["Tools/ai/compose_heap_final_proposals/cli.py"])
        decision = payload["operator_decision"]["decision"]
...
def test_accepted_proposal_gives_patchable() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="composer-decision-") as tmp:
        run_dir = Path(tmp) / "run"
        proposal_dir = run_dir / "team_context" / "proposal_iterations"
        target = "Tools/ai/compose_heap_final_proposals/cli.py"
        text = (
            f"TARGET_FILES: {target}\n\n"
            "IMPLEMENTATION_CHANGES:\n"
            "- Wire operator_decision into the composer output.\n\n"
            "VALIDATION_COMMANDS:\n"
            "- python -m py_compile Tools/ai/compose_heap_final_proposals/cli.py\n"
        )
