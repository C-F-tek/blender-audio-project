...
def main() -> int:
    ...
    require("def source_allowlist_contract" in gate, "gate must expose source allowlist contract", errors)
    require("SOURCE_PATH_ALLOWLIST_CONTRACT" in contract_text, "GPU1 prompt must include hard source allowlist contract", errors)
    require("TARGET_FILES deve essere copiato esattamente da Allowed source paths" in gate, "GPU1 prompt must force exact allowlist target copy", errors)
    require("Rejected/non-allowlisted source refs blacklist" in gate, "provider refinement must blacklist rejected source refs", errors)
