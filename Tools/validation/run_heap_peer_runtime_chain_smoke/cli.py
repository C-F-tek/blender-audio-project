...
STAMP = "heap_peer_runtime_chain_smoke_20990101-010203"
...
def main() -> int:
    ...
    parser.add_argument("--output", default="output/validation/heap_peer_runtime_chain_smoke.json")
    args = parser.parse_args()

    source_repo = Path(args.repo_root).resolve()
    errors: list[str] = []

    with tempfile.TemporaryDirectory(prefix="heap-peer-runtime-chain-") as tmp_raw:
        ...
        write_json(
            shared_memory,
            {
                ...
                "passed": True,
                "shared_memory": True,
                "heap_exchange_context": {"source_of_knowledge": "heap"},
            },
        )
        ...
        result = subprocess.run(
            [
                sys.executable,
                str(source_repo / "Tools/validation/check_unified_chain_contract/cli.py"),
                "--repo-root",
                str(repo),
                "--stamp",
                STAMP,
                ...
                "--require-heap-peer-runtime",
                "--require-shared-memory-evidence",
                "--output",
                str(output),
            ],
            cwd=repo,
    ...
    final = {
        "schema_version": 1,
        "kind": "heap_peer_runtime_chain_smoke",
