...
def path_policy_errors(
    path: str, *, allow_output: bool = True, allow_index_artifacts: bool = True
) -> list[str]:
    ...
    if normalized.startswith("indexAI/") and not (
        allow_index_artifacts and normalized in ALLOWED_INDEXAI_ARTIFACTS
    ):
...
def require_path_if_requested(
    ...
    *,
    ...
    if not value:
        ...
        return
    for error in path_policy_errors(
        value, allow_output=allow_output, allow_index_artifacts=allow_index_artifacts
    ):
...
def validate_manifest(path: Path, repo_root: Path) -> dict[str, Any]:
    ...
    if not isinstance(context_files, list) or not all(
        isinstance(item, str) and item for item in context_files
    ):
    ...
    if not isinstance(enrichment_requested, dict):
        ...
        missing_req = [
            field
            for field in REQUIRED_ENRICHMENT_REQUESTED_FIELDS
            if field not in enrichment_requested
        ]
        if missing_req:
    ...
    if bool_field(enrichment_requested, "build_semantic_chunks"):
        require_path_if_requested(
            errors,
            enrichment_outputs,
            "semantic_chunks_manifest",
            "build_semantic_chunks is true",
            allow_output=False,
        )
        require_path_if_requested(
            errors,
    ...
    if bool_field(enrichment_requested, "select_semantic_chunks"):
        require_path_if_requested(
            errors, enrichment_outputs, "selected_chunks_json", "select_semantic_chunks is true"
        )
        require_path_if_requested(
            errors, enrichment_outputs, "selected_chunks_markdown", "select_semantic_chunks is true"
        )
    ...
    if bool_field(enrichment_requested, "build_selected_chunks_evidence"):
        ...
        require_path_if_requested(
            ...
            "build_selected_chunks_evidence is true",
        )
        require_path_if_requested(
            errors,
            enrichment_outputs,
    ...
    if bool_field(enrichment_requested, "build_context_pack"):
        require_path_if_requested(
            errors, enrichment_outputs, "context_pack_json", "build_context_pack is true"
        )
        require_path_if_requested(
            errors, enrichment_outputs, "context_pack_markdown", "build_context_pack is true"
        )
        require_path_if_requested(
            errors,
            enrichment_outputs,
            "context_pack_evidence_json",
            "build_context_pack is true",
            allow_output=False,
            allow_index_artifacts=False,
        )
    ...
    if bool_field(enrichment_requested, "build_agent_state_packet"):
        require_path_if_requested(
            errors, enrichment_outputs, "agent_state_json", "build_agent_state_packet is true"
        )
        require_path_if_requested(
            errors, enrichment_outputs, "agent_state_markdown", "build_agent_state_packet is true"
        )
        memory_db = normalize_path(
            enrichment_requested.get("memory_db") or enrichment_outputs.get("memory_db")
        )
        if not memory_db:
    ...
    if data.get("provider_execution_requested") is True:
        for flag in (
            ...
            warnings.append(
                "provider_execution_requested is true but no explicit provider/multistep flag is true"
            )
    ...
    return {
        ...
        "provider_execution_performed_by_adapter": data.get(
            "provider_execution_performed_by_adapter"
        ),
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Manifest path. Repeatable or comma-separated.",
    )
    ...
    if not raw_paths:
        raw_paths = [
            "output/local_ai_runs/latest/pipeline/local_ai_task_pipeline_adapter_manifest.json"
        ]
