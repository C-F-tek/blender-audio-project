...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        default="output/validation/runtime_evidence_correlation_launcher_wiring_smoke.json",
    )
    args = parser.parse_args()

    repo_root = Path(args.repo_root).resolve()
    workflow = read_text(repo_root / "Tools/workflow/_powershell/run_unified_local_ai_refactor.ps1")
    wrapper = read_text(repo_root / "Tools/workflow/_powershell/run_unified_real_product_pr.ps1")
    preflight = read_text(repo_root / "Tools/validation/run_real_product_preflight_gate/cli.py")
    heap_runtime_completeness_gate_smoke = read_text(
        repo_root / "Tools/validation/run_heap_runtime_completeness_gate_smoke/cli.py"
    )
    ...
    checks: dict[str, bool] = {
        ...
        "launcher_has_final_marker": "IA-CARMINE-RUNTIME-EVIDENCE-CORRELATION-FINAL-BEGIN"
        in workflow,
        ...
        "preflight_validates_wiring_smoke": "run_runtime_evidence_correlation_launcher_wiring_smoke.py"
        in preflight,
        "preflight_routes_to_heap_runtime_completeness_gate": "run_heap_runtime_completeness_gate_smoke.py"
        in preflight,
        "heap_runtime_completeness_gate_smoke_checks_product_status": "product_status"
        in heap_runtime_completeness_gate_smoke
        and "heap_write_count" in heap_runtime_completeness_gate_smoke,
    }

    for name, passed in checks.items():
        require(
            passed, errors, f"runtime evidence correlation launcher wiring check failed: {name}"
        )
