...
def main() -> int:
    ...
    result = subprocess.run(
        [
            sys.executable,
            str(repo_root / "Tools/validation/provider_mesh/openvino_peer_topology_contract/cli.py"),
            "--repo-root",
            str(repo_root),
            "--output",
            str(contract_json),
    ...
    require(contract.get("passed") is True, errors, "openvino peer topology contract did not pass")
    for key in contract.get("check_order") or []:
        require(contract.get(key) is True, errors, f"openvino peer topology check failed: {key}")
    require(contract_md.exists(), errors, "openvino peer topology markdown report missing")

    report = {
        "schema_version": 1,
        "kind": "openvino_peer_topology_contract_smoke",
