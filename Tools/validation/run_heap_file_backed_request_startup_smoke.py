...
def main() -> int:
    ...
    require(
        '"--request-file",\n            str(heap_request_file),' in launcher,
    ...
    require(
        'parser.add_argument("--request-file"' in prepare,
        "startup reload must accept --request-file",
        errors,
    )
    require(
        'startup_request_file = output_dir / "heap_startup_request.md"' in prepare,
        "startup reload must materialize startup request file",
        errors,
    )
    require(
        'startup_raw_file_list = output_dir / "startup_context_raw_files.txt"' in prepare,
        "startup reload must materialize context raw-file list",
        errors,
    )
    require(
        '"--raw-file-list",\n        str(startup_raw_file_list),' in prepare,
    ...
    require(
        'parser.add_argument("--request-file"' in gate,
    ...
    require(
        'parser.add_argument("--memory-note-file"' in transient,
        "transient context tool must accept memory note files",
        errors,
    )
    require(
        'parser.add_argument("--raw-file-list"' in transient,
    ...
    require(
        'parser.add_argument("--content-file"' in sqlite_memory,
