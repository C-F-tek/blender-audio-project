...
REQUIRED_TOKENS = {
    "python_control_package": "run_agent_review_full_toolbox_decision_loop",
    "python_module_dispatch": "Tools.workflow",
    ...
    "line_count_reuse": "build_python_line_count_csv",
    "code_interpreter_reuse": "build_code_interpreter_report",
    "gpu_npu_orchestrator_reuse": "gpu_npu_parallel_orchestrator",
    "decision_loop_reuse": "run_agent_review_decision_loop.py",
    "post_validation_packet_reuse": "run_post_validation_ai_packet.ps1",
    "shared_toolbox_bundle_reuse": "shared_toolbox_bundle",
    "heap_runtime_product_reuse": "build_heap_runtime_product_package",
    "patch_notes_quality_product": "build_patch_notes_quality_product",
    ...
    "scoped_validation_contract": "check_validation_report_contract",
...
def run_smoke(repo_root: Path, workflow_value: str) -> dict[str, Any]:
    ...
    if not workflow.exists():
        ...
        python_runner = engine_dir / "cli.py"
        engine_modules = sorted(engine_dir.glob("py_*.py")) if engine_dir.exists() else []
        for candidate in (python_runner, *engine_modules):
            if candidate.exists():
    ...
    if (
        "--run-npu-auditor-provider" in text
        and "if ($RunLegacyNpuAuditorProvider)" not in text
        and "RunLegacyNpuAuditorProvider" not in text
    ):
        errors.append(
