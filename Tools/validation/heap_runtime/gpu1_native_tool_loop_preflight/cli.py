...
from Tools.validation.heap_runtime.gpu1_native_tool_loop_preflight.context import build_preflight_prompt, native_tool_required_retry_message, startup_context, tool_loop_soft_stop_message
from Tools.validation.heap_runtime.gpu1_native_tool_loop_preflight.cli_args import validate_explicit_preflight_args
...
from Tools.validation.heap_runtime.gpu1_native_tool_loop_preflight.loop_io import (
    ConsumptionOwner,
    assistant_after_tool_message,
    assistant_history_message,
    broker_args,
    has_history_shape,
    json_write,
    load_operator_prompt,
    operator_prompt_ref,
    tool_calls,
    tool_result_message,
)
...
from Tools.validation.heap_runtime.gpu1_native_tool_loop_preflight.readable import render_delta_readable
...
def _run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    args.operator_prompt = load_operator_prompt(args, repo_root)
    ...
    prompt = build_preflight_prompt(
        ...
        args.max_subturns,
        args.model,
        operator_prompt_ref(args, repo_root),
    )
    ...
    for subturn in range(max(1, int(args.max_subturns))):
        ...
        history.append(assistant_history_message(report))
        calls = tool_calls(report)
        if not calls:
            ...
            candidate_consumption = gpu1_tool_result_consumption_state(
                ConsumptionOwner(repo_root, all_results),
                events,
        ...
        json_write(packet, request_file)
        broker = build_report(
            broker_args(
                repo_root,
                request_file,
        ...
        for index, request in enumerate(requests):
            ...
            history.append(tool_result_message(repo_root, result, str(request["tool_call_id"])))
        json_write(history, history_path)
    else:
    ...
    if all_results and (
        ...
        history.append(assistant_history_message(final_report))
        json_write(history, history_path)
...
def _report(
    ...
    final_nativetool_calls = int(subturn1.get("native_tool_call_count") or 0) if subturn1 else 0
    operator_delta_valid = bool(operator_delta.get("passed"))
    tool_loop_closed = bool(
        subturn1
        and final_nativetool_calls == 0
    ...
    assistant_text_ref = write_text_artifact(
        ...
        text=str(assistant_after_tool_message(history).get("content") or ""),
    ...
    report = {
        ...
        "native_tool_call_count": total_nativetool_calls,
        ...
        "chat_history_has_assistant_tool_call_tool_and_resume": has_history_shape(history),
...
def main() -> int:
    ...
    parser.add_argument("--max-subturns", type=int, default=0)
