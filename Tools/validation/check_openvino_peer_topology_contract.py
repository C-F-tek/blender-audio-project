...
def build_report(repo_root: Path) -> dict[str, Any]:
    ...
    runtime_mesh = read_text(
        repo_root / "Tools/validation/check_real_product_runtime_mesh_contract.py"
    )
    ...
    checks: dict[str, bool] = {
        ...
        "gpu0_peer_command_is_support_lane": has(
            gpu0_peer, "build_openvino_gpu0_workload_report.py"
        )
        ...
        "npu_micro_uses_runtime_context_and_tool_broker": has(
            npu_micro, "collect_runtime_tool_context_reports"
        )
        ...
        "budget_governor_defines_lanes": exists(
            repo_root, "Tools/ai/heap_provider_budget_governor.py"
        )
        ...
        "invocation_contract_defines_telemetry": exists(
            repo_root, "Tools/ai/heap_provider_invocation_contract.py"
        )
        ...
        "orchestrator_launches_gpu0_and_npu_as_peers": has(
            orchestrator, "launch_due_gpu0_peer_supports"
        )
        ...
        "real_product_wrapper_requests_openvino_npu_peer_lanes": has(
            wrapper, "-RunOpenVinoGpu0Workload"
        )
    ...
    errors = [
        f"missing openvino peer topology contract: {name}"
...
def main() -> int:
    ...
    parser.add_argument(
        "--output", default="output/validation/openvino_peer_topology_contract.json"
    )
