...
These tools do not replace Blender runtime packages and do not modify raw
analysis JSON files.
...
```powershell
py .\Tools\ai\build_music_intermediates.py `
  --analysis-json .\output\track_analysis.json `
  --output-dir .\output\ai_pipeline
```
...
Run the safe orchestrator in dry-run mode:
