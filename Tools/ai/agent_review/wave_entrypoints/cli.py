...
def review_file(path: Path, repo: Path) -> dict[str, Any]:
    ...
    suggested: list[str] = []
    positives: list[str] = []
    role = target_role(path.relative_to(repo) if path.is_relative_to(repo) else path)
    expects_cli = role.endswith("_cli")
    ...
    if expects_cli and has_argparse(text):
    ...
    elif role == "analyze_cli":
        for token in ["--skip-music-context", "--run-ollama-agent", "--output-dir", "--fps"]:
            if token in text:
                positives.append(f"cli_token_present:{token}")
            else:
                attention.append(f"cli_token_missing:{token}")
    elif role == "summary_impl":
        required_tokens = [
            "energy_profile",
    ...
    elif role == "summary_cli":
        for token in ["--analysis-json", "--out-json", "--skip-music-context"]:
            if token in text:
                positives.append(f"cli_token_present:{token}")
            else:
                attention.append(f"cli_token_missing:{token}")
