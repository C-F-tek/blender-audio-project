...
```powershell
python -m Tools.ai <tool> [tool args...]
```

The source of truth for public tool names is `Tools/ai/dispatch.py`. Do not infer supported commands from old root-level script names; use the dispatcher or `python -m Tools.ai --list`.
...
GPU0/OpenVINO -> reviewer/refiner/peer workload lane
...
- Register public tool names in `Tools/ai/dispatch.py`.
