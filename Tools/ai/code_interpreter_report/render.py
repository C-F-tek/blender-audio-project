...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    for item in report.get("largest_files", [])[:20]:
        lines.append(
            f"- `{item.get('path')}` - `{item.get('line_count')}` lines, risk `{item.get('risk')}`"
        )
    lines.append("")
    ...
    for item in recommendations[:40]:
        lines.append(
            f"- `{item.get('id')}` `{item.get('target_file')}` risk `{item.get('risk')}`: {', '.join(item.get('reasons') or ['no reason recorded'])}"
        )
    lines.append("")
