...
def gpu_summary(orchestrator: dict[str, Any], gpu_report: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "gpu_recommendation_count": int_field(
            gpu_report, "recommendation_count", "filtered_recommendation_count"
        )
        ...
        "evidence_ready_for_manual_patch_count": int_field(
            gpu_report, "evidence_ready_for_manual_patch_count"
        )
        ...
        "provider_execution_performed": bool(
            ...
            or gpu_report.get("provider_execution_performed")
        ),
        "patch_application_performed": bool(
            orchestrator.get("patch_application_performed")
...
def build_report(repo_root: Path, orchestrator_path: Path, gpu_report_path: Path) -> dict[str, Any]:
    ...
    if summary and summary.get("gpu_recommendation_count", 0) > 0:
        warnings.append(
            "GPU report already contains recommendations; diagnostic fallback not emitted"
        )
...
def main() -> int:
    ...
    report = build_report(
        repo_root,
        resolve_path(repo_root, args.orchestrator),
        resolve_path(repo_root, args.gpu_report),
    )
        ...
        json.dumps(
            {
                ...
                "repair_failure_recommendation_emitted": report["decision"][
                    "repair_failure_recommendation_emitted"
                ],
