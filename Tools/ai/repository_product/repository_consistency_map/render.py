...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines.append(
        f"- Counted text-like files: `{report['scope'].get('repository_line_count_available_count')}`"
    )
    lines.append(
        f"- Total counted text lines: `{report['scope'].get('repository_text_line_count_total')}`"
    )
    ...
    if len(report.get("findings", [])) > 200:
        lines.append(
            f"| ... | ... | ... | ... | ... | truncated in Markdown; JSON contains {len(report['findings'])} findings |"
        )
    lines.append("")
