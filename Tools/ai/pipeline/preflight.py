...
def preflight(repo: Path, out: Path, args: Any) -> dict[str, Any]:
    ...
    agent_packet = (
        Path(args.agent_state_packet).resolve()
        if getattr(args, "agent_state_packet", None)
        else None
    )
    ...
    if args.gpu_command and "{output}" not in args.gpu_command:
        warnings.append(
            "GPU command does not include {output}; planner output may not be captured consistently."
        )

    blender_doc = repo / "docs" / "LOCAL_WORKSTATION_TARGET.md"
    if (
        blender_doc.exists()
        and "blender command is not currently available in PATH"
        in blender_doc.read_text(encoding="utf-8", errors="replace")
    ):
        warnings.append(
            "Local workstation profile says blender is not in PATH; use full Blender executable path for CLI tests."
        )
