...
def visual_directive(name: str, intensity: float | None, event_count: int) -> dict[str, Any]:
    ...
    return {
        ...
        "camera": "slow reveal" if name == "intro" else "impact moves on peaks" if level == "high" else "stable rhythmic motion",
        "lighting": "soft establishing light" if name == "intro" else "burst accents" if event_count else "controlled ambient variation",
        "materials": "subtle shader modulation" if level == "low" else "strong emissive/audio-reactive modulation",
...
def mapping_candidates(segs: list[dict[str, Any]], bpm: float | None, peaks: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [
        {
...
def assumptions(duration: float, bpm: float | None, primary_name: str | None, series_count: int) -> list[str]:
    out = []
    if not duration:
    ...
    if series_count:
        out.append("Primary intensity series was selected heuristically from available numeric series.")
    out.append("Full frame-level analysis JSON remains read-only and is not rewritten by this tool.")
    return out
    ...
    artifacts = {
        "track_summary.json": {
            ...
            "ai_readiness": {
                ...
                "score": round(sum([bool(duration), bool(bpm), bool(primary_name), bool(segs)]) / 4, 3),
            },
            "assumptions": ass,
        },
        "music_segments.json": {"schema_version": 2, "generated_at": now, "source_analysis": str(source), "segments": segs},
        ...
        "ai_scene_brief.json": {
            ...
            "track_facts": {"duration_sec": round(duration, 3) if duration else None, "estimated_bpm": round(bpm, 3) if bpm else None, "segment_count": len(segs), "primary_series_trend": trend(primary)},
            "recommended_visual_progression": [{"segment": s["name"], "start_sec": s["start_sec"], "end_sec": s["end_sec"], "visual_directive": s["visual_directive"]} for s in segs],
            "constraints": ["Do not overwrite full analysis JSON files.", "Keep generated scripts configurable.", "Validate before accepting generated code.", "Avoid ShaderNodeTexMusgrave in Blender 5.x."],
            "assumptions": ass,
        },
        "ai_resource_budget.json": {
            ...
            "recommendations": {"max_parallel_npu_jobs": 4, "gpu_role": "main generation and merge", "npu_role": "short review and scoring", "cpu_role": "validation and orchestration", "avoid_concurrent_heavy_render_and_large_llm": True},
        },
