...
from Tools.ai.patch_plan_quality_product.io_utils import flatten_json, read_text, repo_rel

TOKEN_RE = re.compile(r'[A-Za-z0-9_./:-]{4,}')
BANNED = {'true','false','none','null','output','validation','report','json','markdown','patch','plan','tools','python','manual','review','schema','version','passed','errors','warnings'}
...
def ensure_fts(conn: sqlite3.Connection) -> bool:
    try:
        conn.execute('CREATE VIRTUAL TABLE IF NOT EXISTS evidence_fts USING fts5(source, kind, content)')
        return True
    except sqlite3.OperationalError:
        conn.execute('CREATE TABLE IF NOT EXISTS evidence_fts_fallback (rowid INTEGER PRIMARY KEY AUTOINCREMENT, source TEXT NOT NULL, kind TEXT NOT NULL, content TEXT NOT NULL)')
        return False

def index_doc(conn: sqlite3.Connection, fts_enabled: bool, source: str, kind: str, content: str) -> None:
    if not content.strip():
        return
    if fts_enabled:
        conn.execute('INSERT INTO evidence_fts(source, kind, content) VALUES (?, ?, ?)', (source, kind, content))
    else:
        conn.execute('INSERT INTO evidence_fts_fallback(source, kind, content) VALUES (?, ?, ?)', (source, kind, content))

def search_docs(conn: sqlite3.Connection, fts_enabled: bool, query: str, limit: int) -> list[dict[str, Any]]:
...
def build_search_index(conn: sqlite3.Connection, repo_root: Path, input_paths: dict[str, Path], loaded: dict[str, dict[str, Any]], request: str, extra_context: list[str]) -> tuple[bool, int]:
    ...
    for extra in extra_context:
        ...
        if content:
            index_doc(conn, fts_enabled, repo_rel(extra_path, repo_root), 'extra_context', content)
            indexed += 1
    conn.commit()
