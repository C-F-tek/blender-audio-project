...
FORBIDDEN_PREFIXES = ('output/', 'renders/', '.git/', 'indexAI/code_chunks/', 'indexAI/project_code_chunks/')
...
def has_useful_text_or_dict(value: Any, min_chars: int = 24) -> bool:
    return (isinstance(value, dict) and bool(value)) or (isinstance(value, str) and len(value.strip()) >= min_chars)
...
def score_plan(plan: dict[str, Any], repo_root: Path, index: int) -> dict[str, Any]:
    ...
    targets = [str(item) for item in plan.get('target_files', [])] if isinstance(plan.get('target_files'), list) else []
    ...
    checks = {
        ...
        'has_validation_commands': isinstance(plan.get('validation_commands'), list) and bool(plan.get('validation_commands')),
        'has_stop_conditions': isinstance(plan.get('stop_conditions'), list) and bool(plan.get('stop_conditions')),
        'manual_review_marked': plan.get('manual_review_required') is True or str(plan.get('status', '')).lower() == 'ready_for_manual_review',
    }
    weights = {'has_existing_targets': 20, 'has_rationale': 15, 'has_edit_strategy': 20, 'has_source_evidence': 15, 'has_validation_commands': 15, 'has_stop_conditions': 10, 'manual_review_marked': 5}
    ...
    notes.extend(f'missing_or_weak:{key}' for key in weak if key != 'has_existing_targets' or not target_notes)
    return {'id': plan_id, 'score': min(score, 100), 'target_files': targets, 'valid_target_count': valid_targets, 'checks': checks, 'notes': notes}

def classify(fatal_errors: list[str], quality_gate_passed: bool, avg_score: float, hit_count: int) -> str:
    if fatal_errors:
