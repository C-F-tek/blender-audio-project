...
from Tools.ai.operator_product_core import LauncherConfig, profile_names, select_profile
...
from Tools.ai.run.cli import DEFAULT_PROFILE, default_final_root, default_task_md
...
class OperatorRunView:
    ...
    def _build(self) -> None:
        ...
        ttk.Label(frame, text="Profile").grid(row=6, column=0, sticky="w", pady=2)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args and args[0] in {"--help", "-h"}:
        print("Usage: python -m Tools.ai operator_product_gui")
        print("Graphical view for the same controller used by python -m Tools.ai run.")
        return 0
    root = tk.Tk()
