...
ALLOWED_PREFIXES = ("Tools/ai/", "Tools/validation/", "Tools/docs/", "Tools/workflow/")
DENY_PREFIXES = (".git/", "output/", "renders/", "indexAI/", "Scripting/")
DENY_TOKENS = ("ollama", "openvino", "blender", "ffmpeg", "provider_probe")
...
def normalize_repo_path(repo_root: Path, raw: str) -> tuple[str, str]:
    ...
    if any(lower.startswith(prefix.lower()) for prefix in DENY_PREFIXES):
        return rel, "path prefix is forbidden"
    if any(token in lower for token in DENY_TOKENS):
        return rel, "provider/application runtime token is forbidden"
    if not any(rel.startswith(prefix) for prefix in ALLOWED_PREFIXES):
        return rel, "path is outside virtual-dev allowlist"
