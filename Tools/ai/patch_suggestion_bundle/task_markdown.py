...
def suggestion_items_from_blocks(blocks: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for block in blocks:
        ...
        if isinstance(raw_items, list):
            ...
            suggestions.append({key: value for key, value in block.items() if not key.startswith("_")})
    return suggestions
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    for item in report.get("suggestions") or []:
        lines.append(f"- `{item.get('id') or item.get('proposal_id') or ''}` -> `{item.get('target_file') or item.get('path') or ''}`")
