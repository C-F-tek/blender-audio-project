...
This tool does not execute the heap runtime. It only externalizes launcher
variables and prints/writes a command that an operator can review and run.
"""
...
def list_profiles(profile_doc: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "profiles": [
            {"name": name, "description": value.get("description", "") if isinstance(value, dict) else ""}
