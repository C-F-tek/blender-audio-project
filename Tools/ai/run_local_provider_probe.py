...
def build_heap_patch_proposal_prompt(prompt: str) -> str:
    ...
    return (
        ...
        "Return one Markdown block for the NEXT concrete heap delta only.\n"
        "Use pointer history as working memory; emit only the current patchable slice.\n"
        "GPU0_REVIEW_REQUEST and NPU_AUDIT_REQUEST must ask for concrete checks.\n\n"
        "# HEAP_DELTA_PROPOSAL\n"
        ...
        "TARGET_FILES:\n"
        "Emit only exact repo-relative paths copied from SOURCE_PATH_ALLOWLIST_CONTRACT.\n"
        "If no allowlisted path is patchable, use EXIT_DECISION=NO_PATCHABLE_TARGET and omit PATCH_SKETCH.\n\n"
        "Required sections: PROBLEM, EVIDENCE, IMPLEMENTATION_CHANGES, PROPAGATION_TASKS, BACKLOG_TASKS, GPU0_REVIEW_REQUEST, NPU_AUDIT_REQUEST, PATCH_SKETCH, VALIDATION_COMMANDS, RISKS.\n"
        "PATCH_SKETCH must contain a real unified diff only for an exact TARGET_FILES path.\n"
        "VALIDATION_COMMANDS must name concrete commands for the exact TARGET_FILES paths.\n\n"
        "EXIT_DECISION_RULES:\n"
        ...
        "- Never emit real_existing_file.py, tools/... paths, docs/LOCAL_VALIDATION_EVIDENCE/, output/, indexAI/, or rejected-evidence paths.\n"
