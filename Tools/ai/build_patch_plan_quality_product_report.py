...
def main() -> int:
    ...
    print(json.dumps({'passed': report['passed'], 'quality_gate_passed': report['quality_gate_passed'], 'classification': report['classification'], 'output': str(output), 'markdown': str(markdown), 'fallback_path_note_count': len(report['fallback_path_notes']), 'patch_plan_count': report['quality']['patch_plan_count'], 'average_plan_score': report['quality']['average_plan_score'], 'sqlite_fts5_enabled': report['quality']['sqlite_fts5_enabled'], 'fts_total_hit_count': report['quality']['fts_total_hit_count']}, indent=2, ensure_ascii=False))
