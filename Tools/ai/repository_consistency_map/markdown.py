...
def scan_markdown_file_task(task: tuple[str, str, dict[str, str], int]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[str]]:
    ...
    *,
    ...
    if actual_backend in {"process", "thread"}:
        ...
        with executor_class(max_workers=worker_count) as executor:
            for file_references, file_commands, file_warnings in executor.map(scan_markdown_file_task, tasks):
