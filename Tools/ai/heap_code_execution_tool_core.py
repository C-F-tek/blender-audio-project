...
ALLOWED_VALIDATION_PREFIXES = ("Tools/validation/",)
DENY_PREFIXES = (
    ".git/",
    "output/",
    "renders/",
...
def normalize_repo_path(repo_root: Path, raw: str) -> tuple[str, str]:
    ...
    candidate = resolve_path(repo_root, text)
    try:
        rel = candidate.relative_to(repo_root.resolve(strict=False)).as_posix()
    except ValueError:
        return text, "path escapes repository root"
    lower = rel.lower()
    for prefix in DENY_PREFIXES:
        if lower.startswith(prefix.lower()):
            return rel, f"path prefix is forbidden: {prefix}"
    for suffix in DENY_SUFFIXES:
        if lower.endswith(suffix):
            return rel, f"path suffix is forbidden: {suffix}"
    if not candidate.exists():
        return rel, "path does not exist"
    return rel, ""
...
def validate_target(repo_root: Path, raw: str) -> tuple[str, str]:
    ...
    if error:
        return rel, error
    if not any(rel.startswith(prefix) for prefix in ALLOWED_TARGET_PREFIXES):
        return rel, "target is outside heap code execution prefixes"
    return rel, ""
...
def validate_validation_script(repo_root: Path, raw: str) -> tuple[str, str]:
    ...
    if not rel.endswith(".py"):
        return rel, "validation script must be Python"
    if not any(rel.startswith(prefix) for prefix in ALLOWED_VALIDATION_PREFIXES):
        return rel, "validation script must live under Tools/validation/"
    if any(token in lower for token in DENY_RUNTIME_TOKENS):
        return rel, "validation script appears to target provider/application runtime"
    return rel, ""
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    for key in ("request_file", "debug_lab_report", "debug_lab_markdown"):
        lines.append(f"- {key}: `{report.get(key)}`")
