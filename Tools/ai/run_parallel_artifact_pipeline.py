...
def workstation_context(repo: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "summary": text[:1200] if text else None,
    }
...
def main() -> int:
    ...
    if args.build_chunks:
        serial.append((
            step_meta("build_semantic_code_chunks", "CPU", "Build symbol-aware repository context for AI retrieval.", EXPECTED_CHUNK_ARTIFACTS),
            [py, script("Tools/npu/build_semantic_code_chunks.py"), "--repo-root", str(repo)],
        ))
    if args.build_music_summary:
        serial.append((
            step_meta("build_music_intermediates", "CPU", "Build compact AI-friendly music artifacts from the full analysis JSON.", EXPECTED_MUSIC_ARTIFACTS),
            [py, script("Tools/ai/build_music_intermediates.py"), "--analysis-json", str(Path(args.analysis_json).resolve()), "--output-dir", str(out)],
        ))
    if args.use_npu:
        parallel.append((
            step_meta("npu_artifact_review", "NPU", "Review compact artifacts for schema, size, blocked patterns, and obvious risks.", [str(out / "npu_artifact_review.json")]),
            [py, script("Tools/npu/run_npu_artifact_reviewer.py"), "--input", str(out), "--output", str(out / "npu_artifact_review.json"), "--max-workers", str(args.npu_workers)],
        ))
    if args.gpu_command:
        parallel.append((
            step_meta("gpu_command", "GPU", "Run optional heavy planner/generator command.", [str(out / "gpu_planner_output.json")]),
            shlex.split(args.gpu_command.format(brief=str(out / "ai_scene_brief.json"), output=str(out / "gpu_planner_output.json"))),
        ))
    if args.validate:
        serial.append((
            step_meta("validate_ai_artifacts", "CPU", "Validate generated artifacts and apply task-capsule guardrails.", [str(out / "ai_validation_report.json")]),
            [py, script("Tools/ai/validate_ai_artifacts.py"), "--repo-root", str(repo), "--artifact-dir", str(out), "--output", str(out / "ai_validation_report.json"), "--allow-errors"],
        ))
    ...
    report = {
        ...
        "lanes": {
            "CPU": [r["name"] for r in results if r.get("lane") == "CPU"],
            "NPU": [r["name"] for r in results if r.get("lane") == "NPU"],
