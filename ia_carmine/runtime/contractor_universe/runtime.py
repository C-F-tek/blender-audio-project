...
from ia_carmine.runtime.runtime_universe.builder import RepoRuntimeUniverseBuilder
...
class ContractorUniverseRuntime:
    """Compact scheduler surface for the single Universo IA runtime."""
    ...
    def run(self) -> dict[str, Any]:
        ...
        report = {
            ...
            "universe_scope": "single_canonical_universe_component",
            "artifact_classification": "compact_scheduler_evidence_not_product",
            "canonical_run_integration_required": True,
            ...
            "time": {
                ...
                "budget_seconds": self.clock.budget_seconds,
                "soft_close_after_seconds": self.clock.soft_close_after_seconds,
                "tick_seconds": self.clock.tick_seconds,
                "time_counter_contract": self.clock.time_counter_contract,
            },
    ...
    def _render_markdown(
        ...
        lines = [
            ...
            f"- Artifact classification: `{report['artifact_classification']}`",
            f"- Canonical run integration required: `{report['canonical_run_integration_required']}`",
            f"- Time semantics: `{report['time']['time_counter_contract'].get('operational_semantics')}`",
            f"- Soft close truncates pointer recursion: `{not report['time']['time_counter_contract'].get('soft_close_must_not_truncate_pointer_recursion')}`",
