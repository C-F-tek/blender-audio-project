...
def _external_heap_contract(external_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    provider_rejections = (
        _list_or_empty(pointer.get("provider_rejections"))
        or _list_or_empty(revision.get("provider_rejections"))
        or _list_or_empty(stats.get("provider_rejections"))
    )
    provider_rejection_reasons = (
        _list_or_empty(pointer.get("provider_rejection_reasons"))
    ...
    return {
        ...
        "provider_execution_performed": bool(
            long_response.get("provider_execution_performed")
        ...
        "provider_verified_count": _safe_int(pointer.get("provider_verified_count") or revision.get("provider_verified_count")),
        "provider_rejected_count": _safe_int(pointer.get("provider_rejected_count") or revision.get("provider_rejected_count")),
        ...
        "resume_from_block_id": str(
            stats.get("resume_from_block_id")
            or revision.get("resume_from_block_id")
            or pointer.get("resume_from_block_id")
            or ""
        ),
        "latest_block_id": str(
            stats.get("latest_block_id")
            or revision.get("latest_block_id")
            or pointer.get("latest_block_id")
            or ""
        ),
    ...
    if getattr(args, "allow_provider_generation", False):
        ...
        if external_contract.get("provider_rejection_reasons"):
            errors.append(
                ...
                + ",".join(str(item) for item in external_contract.get("provider_rejection_reasons") or [])
            )
