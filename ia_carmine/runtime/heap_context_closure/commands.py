...
def heap_command(args: Any, state: dict[str, Any]) -> list[str]:
    command = [
        state["project_python"],
        "-m",
        "ia_carmine.runtime.heap_runtime.completeness_gate.cli",
        "--repo-root",
        ".",
        *request_args(state),
        "--objective",
        ...
        str(getattr(args, "ollama_context_candidates", "") or ""),
        "--gpu1-base-url",
        str(getattr(args, "gpu1_base_url", "") or ""),
        "--gpu0-model",
        str(getattr(args, "gpu0_model", "") or ""),
        "--gpu0-base-url",
        str(getattr(args, "gpu0_base_url", "") or ""),
        "--gpu0-vulkan-visible-devices",
        str(getattr(args, "gpu0_vulkan_visible_devices", "") or ""),
        "--max-new-tokens",
        str(args.max_new_tokens),
        "--gpu0-max-new-tokens",
        str(getattr(args, "gpu0_max_new_tokens", 0) or 0),
        "--max-context-files",
        ...
        str(getattr(args, "tool_inventory_roots", "") or ""),
        "--semantic-path-boosts",
        str(getattr(args, "semantic_path_boosts", "") or ""),
        "--ai-context-pack-profile",
        str(getattr(args, "ai_context_pack_profile", "") or ""),
        "--code-interpreter-inputs",
        str(getattr(args, "code_interpreter_inputs", "") or ""),
        "--duplication-audit-roots",
        str(getattr(args, "duplication_audit_roots", "") or ""),
        "--provider-prompt-tool-catalog-cap",
        str(getattr(args, "provider_prompt_tool_catalog_cap", 0) or 0),
        "--keep-alive",
        str(args.keep_alive),
        "--gpu0-iterations",
        ...
        str(getattr(args, "npu_micro_start_mode", "") or ""),
        "--npu-micro-timeout-seconds",
        str(args.npu_micro_timeout_seconds),
        "--npu-final-wait-seconds",
        str(getattr(args, "npu_final_wait_seconds", 0) or 0),
        ...
        "--max-degraded-lanes",
        str(getattr(args, "max_degraded_lanes", 0) or 0),
        "--canonical-run-metadata",
        str(getattr(args, "canonical_run_metadata", "") or ""),
        "--canonical-run-fingerprint",
        str(getattr(args, "canonical_run_fingerprint", "") or ""),
        "--output-dir",
        str(state["run_dir"]),
        "--output",
