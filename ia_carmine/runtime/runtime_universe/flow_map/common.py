...
DEFAULT_ENTRYPOINT = "ia_carmine/providers/provider_mesh/gpu_npu_parallel_orchestrator/cli.py"
STAMP_RE = re.compile(r"\d{8}-\d{6}")
CANONICAL_COMPONENTS = {
    "orchestrator": {"type": "process", "label": "orchestrator"},
    "gpu_planner": {"type": "provider", "label": "GPU1 / Ollama planner"},
    "gpu0_peer": {"type": "provider", "label": "GPU0 / Ollama Vulkan peer"},
...
def component_from_report(path: str, data: dict[str, Any]) -> str:
    ...
    if "parallel_gpu" in lower_path or "gpu_deep" in lower_path or "gpu_planner" in lower_path:
        return "gpu_planner"
    if "gpu0" in lower_path or "ollama_gpu0" in kind or "openvino_gpu0" in kind:
        return "gpu0_peer"
