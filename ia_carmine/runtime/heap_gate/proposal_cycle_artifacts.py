    ...
    *,
    ...
    md = [
        ...
        *[f"- {item}" for item in deterministic_reviews.get("npu_micro_task_piece", [])],
        ...
        f"- Stored as artifact ref: `{data.get('gpu1_free_text_evidence_ref')}`",
        "- JSON stores only ref/hash/chars/tail.",
        ...
        "",
        "## Final product file-read grounding",
        "",
        f"- Requires file-read: `{data.get('final_product_requires_file_read')}`",
        f"- File-read verified: `{data.get('final_product_file_read_verified')}`",
        f"- File-read refs: `{data.get('final_product_file_read_refs')}`",
        f"- Contract errors: `{(data.get('final_product_code_file_read_contract') or {}).get('errors')}`",
        "",
