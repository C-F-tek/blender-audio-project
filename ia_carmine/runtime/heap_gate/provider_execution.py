...
class RuntimeGateProviderExecutionMixin:
    ...
    def run_provider_teamwork(self, round_id: int, revision: int = 0) -> None:
        ...
        try:
            ...
            self.gpu1_leader_started_before_sidecars = True
            ...
            one_turn_gate_passed = (
                leader_report_for_packet.get("gpu1_one_turn_runtime_gate_passed") is True
            )
            if getattr(self.args, "allow_provider_generation", False) and not one_turn_gate_passed:
                reason = str(
                    leader_report_for_packet.get("gpu1_one_turn_blocker")
                    or (
                        "gpu1_one_turn_runtime_gate_missing"
                        if not leader_report_for_packet.get("gpu1_one_turn_runtime_gate_present")
                        else "gpu1_one_turn_runtime_gate_failed"
                    )
                )
                self.sidecars_start_policy = "skipped_gpu1_one_turn_gate_failed"
                ...
                write_provider_launch_manifest(
                    ...
                    revision,
                    time_contract,
                    "sidecars_skipped_gpu1_one_turn_gate_failed",
                )
                return
    ...
    def capture_gpu1_primary_evidence_after_provider_join(
        ...
        from ia_carmine.runtime.heap_gate.provider_primary_evidence import (
            capture_gpu1_primary_evidence_after_provider_join,
        )

        return capture_gpu1_primary_evidence_after_provider_join(
            self, leader_report, round_id
        )
