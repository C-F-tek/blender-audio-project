...
class RuntimeGateProviderContextMixin:
    ...
    def gpu1_request_prompt(self) -> str:
        ...
        return (
            ...
            "GPU0 e NPU sono sidecar packet_review_only: fanno review/audit del packet GPU1 corrente; tu GPU1 resti leader, esegui eventuali jump/backrefinement sul pointer graph e integri i loro veto in un nuovo delta causale. "
            ...
            "Il FINAL_PRODUCT e' uno solo: testo, codice, oppure testo+codice. PLAN_PRODUCT_FULL_PATCH.md e CODE_PRODUCT_FULL_PATCH.md sono superfici di packaging dello stesso FINAL_PRODUCT, non prodotti concorrenti. "
            "Ogni tuo turno produce un FINAL_PRODUCT_DELTA causale che il composer deterministico applichera' con append/replace/supersede/refine; non ci sara' una sintesi GPU finale a fine run. "
            "Non emettere blocked come valore GPU1: se non riesci a produrre un delta valido, il runtime classifica la tua risposta come raw evidence e decide blocked_with_reason. "
            ...
            "CONSUMED_EVIDENCE:\n"
            "- consumed_gpu0_block_id=<id-or-empty>\n"
            "- consumed_npu_block_ids=<ids-or-empty>\n"
            "- tool_or_matrix_refs=<refs-or-empty>\n"
            "NEXT_RUNTIME_INTENT:\n"
            "- <azione concreta per il prossimo ciclo/runtime>\n"
            "FINAL_PRODUCT_DELTA:\n"
            "<testo/codice/testo+codice destinato al FINAL_PRODUCT unico>\n"
            "Valori vietati per GPU1: FINAL_PRODUCT_KIND=blocked, FINAL_PRODUCT_ACTION=blocked, FINAL_PRODUCT_ACTION=block.\n"
            "Per richieste implementative devi produrre anche un blocco operativo, non consigli generici. Usa sezioni TARGET_FILES, PROBLEM, IMPLEMENTATION_CHANGES, PATCH_SKETCH_UNIFIED_DIFF, VALIDATION_COMMANDS, RISKS, EXIT_DECISION. PATCH_SKETCH_UNIFIED_DIFF deve essere un blocco ```diff con diff --git a/<path> b/<path> su path allowlisted; se non hai un target verificato usa EXIT_DECISION=NO_PATCHABLE_TARGET. TARGET_FILES deve essere copiato esattamente da Allowed source paths; non citare path non allowlisted nemmeno in PROBLEM/EVIDENCE/PATCH_SKETCH_UNIFIED_DIFF. La tua proposta e' evidenza: la parte code del FINAL_PRODUCT sara' valida solo se matrix/synthesis estrae e valida il diff.\n"
