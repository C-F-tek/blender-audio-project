...
class RuntimeGateProviderContextMixin:
    ...
    def gpu1_request_prompt(self) -> str:
        ...
        return (
            ...
            "BROKER_NATIVE_TOOL_RULE: GPU1 e' la lane Ollama primaria e puo' guidare broker tools e sintesi finale. GPU0 usa lo stesso schema tool-call ma produce solo peer_refinement, veto, evidence_request o generic_write_capture: ogni suo output richiede un turno GPU1 successivo che lo consuma. Usa generic_write se non puoi ancora produrre codice ma puoi raffinare richiesta/piano; se una lane Ollama risponde senza tool call, la prosa utile viene catturata come generic_write no_tool_capture e resta output leggibile. Usa run_heap_virtual_dev_environment, run_heap_code_execution_matrix, agent_runtime_debug_lab e synthesize_patch_candidates quando il codice richiede prova eseguibile. Dopo tre generic_write consumati puoi chiudere come prodotto raffinato leggibile anche con contenuto codice, ma non fingere patch applicate o source write.\n"
