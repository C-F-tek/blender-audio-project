...
def block_provider_execution_performed(block: dict[str, Any]) -> bool:
    return normalize_bool(block.get("provider_work_verified"))
...
def status_lines(pointer: dict[str, Any], causality: dict[str, Any]) -> list[str]:
    lines = [
        ...
        f"- Provider verified/rejected: `{pointer.get('provider_verified_count')}` / `{pointer.get('provider_rejected_count')}`",
        f"- Provider rejection reasons: `{pointer.get('provider_rejection_reasons')}`",
...
def build_markdown(
    ...
    *,
    ...
    stats = {
        ...
        "provider_rejection_reasons": pointer.get("provider_rejection_reasons"),
    }
    return "\n".join(lines).rstrip() + "\n", stats
