...
def render_run_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- Run label: `{report.get('run_label')}`",
        f"- Request file: `{report.get('request_file')}`",
        f"- Run dir: `{report.get('intermediate_run_dir')}`",
        f"- Code product: `{report.get('code_product')}`",
        f"- Provider generation requested: `{report.get('provider_generation_requested')}`",
        f"- Product kind: `{report.get('product_kind')}`",
        f"- Product status: `{report.get('product_status')}`",
        f"- Product approval: `{report.get('product_approval_status')}`",
        f"- Continuation required: `{report.get('continuation_required')}`",
        f"- Resume from block: `{report.get('resume_from_block_id')}`",
        f"- Product blocked reason: `{report.get('product_blocked_reason')}`",
...
def render_lab_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- Artifact intake performed: `{report.get('artifact_intake_performed')}`",
        f"- Artifact intake skipped reason: `{report.get('artifact_intake_skipped_reason')}`",
        f"- Product kind: `{report.get('product_kind')}`",
        f"- Product status: `{report.get('product_status')}`",
        f"- Product approval: `{report.get('product_approval_status')}`",
        f"- Continuation required: `{report.get('continuation_required')}`",
