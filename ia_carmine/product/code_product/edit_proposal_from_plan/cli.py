...
def infer_edit_payload(
    plan: dict[str, Any],
    edit_kind: str,
    unified_diff: str,
    structured_operation: list[dict[str, Any]] | None,
    ...
    if edit_kind == EDIT_KIND_UNIFIED_DIFF:
        if not unified_diff.strip():
            warnings.append(
                "unified_diff edit requested without diff text; proposal will fail smoke until diff is supplied"
            )
        return unified_diff, [], warnings
    if edit_kind == EDIT_KIND_STRUCTURED:
        if not operations:
            warnings.append(
                "structured_edit requested without operations; proposal will fail smoke until operations are supplied"
            )
    ...
    return {
        ...
        "decision": {
            ...
            "recommended_next_layer": (
                "run_code_edit_proposal_smoke"
                if proposal and not errors
                else "fix_or_select_code_patch_plan"
            ),
        },
...
def build_code_edit_from_plan(
    ...
    if selected_plan:
        ...
        diff_text, operations, payload_warnings = infer_edit_payload(
            selected_plan, edit_kind, unified_diff, structured_operations
        )
    ...
    return build_report(
        repo_root, plan_path, plan_report, selected_plan, proposal, errors, warnings
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--edit-kind",
        choices=[EDIT_KIND_NOOP, EDIT_KIND_STRUCTURED, EDIT_KIND_UNIFIED_DIFF],
        default=EDIT_KIND_NOOP,
    )
    parser.add_argument(
        "--unified-diff",
        default="",
        help="Optional bounded unified diff text for unified_diff proposals.",
    )
    ...
    report = build_code_edit_from_plan(
        repo_root, plan_path, args.plan_id, args.edit_kind, args.unified_diff, None
    )
    print(
        write_json_and_markdown(
            repo_root,
