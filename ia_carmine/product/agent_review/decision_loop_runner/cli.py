...
try:
    from ia_carmine.product.agent_review.patch_plan import build_patch_plan
    from ia_carmine.product.agent_review.patch_plan import (
    ...
    from ia_carmine.product.agent_review.patch_plan import build_patch_plan  # type: ignore
    from ia_carmine.product.agent_review.patch_plan import (
        render_markdown as render_patch_plan_markdown,
    )
...
def build_decision_loop_report(args: argparse.Namespace) -> dict[str, Any]:
    ...
    warnings.extend(
        f"recommendations: {warning}" for warning in recommendation_report.get("warnings", [])
    )
    ...
    if (
        ...
        warnings.extend(
            f"patch_plan: {warning}" for warning in patch_plan_report.get("warnings", [])
        )
