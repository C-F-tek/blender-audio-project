...
from ia_carmine.providers.npu.provider_mesh.ai_service_packet import build_ai_service_packet
from ia_carmine.providers.npu.provider_mesh.ai_service_packet import slugify as packet_slugify
from ia_carmine.providers.npu.provider_mesh.blender_manual_context import build_manual_context
from ia_carmine.providers.npu.provider_mesh.npu_code_context import main as build_code_context
from ia_carmine.providers.npu.provider_mesh.music_context import build_music_context
from ia_carmine.providers.npu.provider_mesh._shared.npu_runtime import (
    DEFAULT_MODEL_DIR,
    DEFAULT_NPU_PYTHON,
    npu_preflight,
...
from ia_carmine.providers.npu.provider_mesh.project_ai_index import (
    PROJECT_INDEX_MD,
    PROJECT_MANIFEST_JSON,
    build_project_ai_index,
)
from ia_carmine.providers.npu.provider_mesh.ollama_music_agent import build_prompt as build_music_prompt
from ia_carmine.providers.npu.provider_mesh.ollama_music_agent import markdown_from_insights
...
try:
    ...
    from ia_carmine.providers.npu.pipeline.artifact_paths import (  # type: ignore
        is_allowed_generated_artifact_path,
        normalize_repo_relative_path,
        validate_legacy_runtime_output_paths,  # type: ignore
    )
    from ia_carmine.providers.npu.pipeline.artifact_writer import (  # type: ignore
        PlannedArtifactWrite,
        write_planned_artifact,
    )
    from ia_carmine.providers.npu.pipeline.context_builder import summarize_music_context  # type: ignore
    from ia_carmine.providers.npu.pipeline.io_utils import (  # type: ignore
        read_json,
        read_optional_json,
        read_text,
        write_json,
    )
    from ia_carmine.providers.npu.pipeline.prompts import (  # type: ignore
        build_creative_scene_prompt_payload,
        build_implementation_retry_payload,
        build_merge_prompt_payload,
    )
    from ia_carmine.providers.npu.pipeline.providers import normalize_provider_preflight_report  # type: ignore
    from ia_carmine.providers.npu.pipeline.validators import validate_implementation_draft_contract  # type: ignore
