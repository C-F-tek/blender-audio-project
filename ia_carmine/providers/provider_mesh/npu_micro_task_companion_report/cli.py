...
def resolve_project_python(repo_root: Path, explicit_python: str = "") -> Path:
    ...
    # Policy: explicit project Python > provider-capable env > repo .venv > blocking report error.
    # No system env or PATH fallback is used here.
    if explicit_python:
        return Path(explicit_python).resolve()
    env_python = str(os.environ.get("IA_CARMINE_PYTHON") or "").strip()
    if env_python:
...
def run_npu_micro_task(
    ...
    if npu_preflight is None:
        return {
            ...
            "error_count": 1,
            "warning_count": 0,
            "errors": ["ia_carmine.providers.npu.provider_mesh._shared.npu_runtime import unavailable"],
            "warnings": [],
...
def run_npu_device_workload(
    ...
    child_code = "\n".join(
        [
            ...
            "    raise RuntimeError('NPU device missing')",
...
def main() -> int:
    try:
        from ia_carmine._shared.npu_micro_task_companion_cli import main as cli_main
