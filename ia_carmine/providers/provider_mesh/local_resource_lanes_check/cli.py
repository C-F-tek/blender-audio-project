...
It is still resource/provider preflight mechanics, not a static file read. The
report keeps provider generation/workload evidence separate from resource probe
mechanics so `provider_execution_performed=false` cannot hide device/service
enumeration.
"""
...
try:
    from ia_carmine.providers.provider_mesh.local_resource_lanes_check.report import (
    ...
    from ia_carmine.providers.provider_mesh.local_resource_lanes_check.report import (  # type: ignore
        RESOURCE_MECHANICS_SEMANTICS,
        render_markdown,
        unavailable_lane,
    )
...
def check_npu_lane(repo_root: Path, timeout: float) -> dict[str, Any]:
    ...
    try:
        from ia_carmine.providers.npu.provider_mesh._shared.npu_runtime import DEFAULT_MODEL_DIR, npu_preflight  # noqa: PLC0415
...
def check_gpu_lane(repo_root: Path, timeout: float) -> dict[str, Any]:
    ...
    started = time.perf_counter()
    try:
        from ia_carmine.providers.npu.provider_mesh._shared.npu_runtime import _parse_last_json_line, _run_python  # noqa: PLC0415
    except Exception as exc:  # noqa: BLE001 - report-only check.
        return unavailable_lane(
            "gpu",
...
def run_checks(repo_root: Path, args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        ...
        "resource_mechanics_performed": bool(lane_reports),
        "resource_probe_performed": any(item.get("resource_probe_performed") for item in lane_reports),
        "resource_mechanics_semantics": RESOURCE_MECHANICS_SEMANTICS,
        "operator_authorization_required": True,
        "mechanical_not_static_read": bool(lane_reports),
        ...
        json.dumps(
            {
                ...
                "resource_mechanics_performed": report["resource_mechanics_performed"],
                "resource_probe_performed": report["resource_probe_performed"],
            },
            indent=2,
        )
    )
