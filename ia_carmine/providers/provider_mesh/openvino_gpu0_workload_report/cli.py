...
from ia_carmine.providers.provider_mesh.hardware_capability.workloads import run_openvino_gpu0_tensor_test
from ia_carmine.providers.provider_mesh.openvino_gpu0_workload_report.markdown import render_markdown
...
def main() -> int:
    ...
    if not report.get("provider_device_verified"):
        report.setdefault("errors", []).append("gpu0_openvino_gpu_provider_unavailable")
        report["diagnostic_only"] = True
        report["provider_execution_performed"] = False
        report["passed"] = False
    ...
    report["provider_model"] = str(tool_loop.get("model_id") or "openvino_gpu0_peer")
    ...
    if (
        ...
        report["native_tool_loop_timeout_warning"] = str(
            tool_loop.get("classification") or "openvino_native_tool_call_missing"
        )
        target = "errors" if report.get("semantic_provider_required") else (
            "warnings" if report.get("provider_device_verified") else "errors"
        )
        report.setdefault(target, []).append(report["native_tool_loop_timeout_warning"])
        if target == "errors":
            report["passed"] = False
    if report["semantic_provider_required"] and not report["semantic_provider_execution_performed"]:
        message = (
            ...
            else "GPU.0 semantic provider model was not discovered/executed."
        )
        target = "errors" if report.get("semantic_provider_required") else (
            "warnings" if report.get("provider_device_verified") else "errors"
        )
    ...
    report.update(
        provider_work_status(
            lane="gpu0_peer",
            report=report,
            default_role="gpu0_reviewer_refiner",
        )
    )
    ...
    report.update(
        provider_replight_fields(
            lane="gpu0_peer", role="gpu0_peer_reviewer_refiner", report=report,
            default_model="openvino_gpu0_peer",
            functionalities=["review", "refine", "openvino_gpu0_workload", "broker_tool_catalog"],
        )
    )
