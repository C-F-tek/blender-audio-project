    ...
    return [
        {
            ...
            "reason": f"GPU0 companion task {task['id']} requests compact evidence context.",
            "args": {},
...
def run_gpu0_workload(
    ...
    _ = iterations, min_seconds
    workload_json = out_dir / f"gpu0_companion_worker_ollama_peer_{stamp}.json"
    workload_md = out_dir / f"gpu0_companion_worker_ollama_peer_{stamp}.md"
    cmd = [
        sys.executable,
        "-m",
        "ia_carmine",
        ...
        str(workload_md),
        "--request",
        f"GPU0/Ollama Vulkan companion worker evidence for stamp {stamp}. Review current deterministic source summaries and ask broker tools if needed.",
    ]
    p = subprocess.run(
        cmd,
        cwd=repo_root,
...
def main() -> int:
    ...
    report = {
        ...
        "semantic_execution_mode": (
            "ollama_vulkan_peer_worker"
        ),
        "gpu0_openvino_used": False,
