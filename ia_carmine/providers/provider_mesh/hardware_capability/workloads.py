...
def _base_report() -> dict[str, Any]:
    return {
        ...
        "provider_device_policy": "openvino_GPU.0_only_cpu_not_provider_gpu1_reserved",
    ...
    try:
        ...
        if "GPU.0" not in devices:
            report["errors"].append(
                "gpu0_openvino_gpu_provider_unavailable"
            )
            report["provider_compute_device"] = "openvino/GPU.0_unavailable"
            report["elapsed_seconds"] = round(time.perf_counter() - started, 6)
            return report
        ...
        report["provider_compute_device"] = "openvino/GPU.0"
        report["provider_device_verified"] = True
