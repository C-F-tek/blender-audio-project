...
from ia_carmine.providers.provider_mesh.hardware_capability.policy import (
    build_hardware_runtime_policy,
    policy_warnings,
)
from ia_carmine.providers.provider_mesh.hardware_capability.probes import (
    cpu_diagnostics,
...
def build_manifest(repo_root: Path) -> dict[str, Any]:
    ...
    capabilities = [
        ...
        capability_entry(
            name="ollama_gpu0_vulkan_peer",
            resource="GPU0",
            role="peer_reviewer_refiner",
            status="available" if policy["ollama_gpu0_vulkan"]["visible"] else "unavailable",
            provider="ollama_vulkan",
            diagnostics={"openvino_visibility": openvino, "policy": policy["ollama_gpu0_vulkan"]},
            workload_allowed=bool(policy["ollama_gpu0_vulkan"]["ollama_workload_required"]),
            exclusive=False,
        ),
        capability_entry(
    ...
    if not openvino.get("available"):
        warnings.append(
            "OpenVINO import/device detection unavailable; NPU and GPU0 visibility diagnostics are unavailable."
        )
    ...
    return {
        ...
        "required_resources_visible": {
            ...
            "GPU0_OLLAMA_VULKAN": bool(policy["ollama_gpu0_vulkan"]["visible"]),
