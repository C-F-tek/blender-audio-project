"""Markdown renderer for NPU micro-task reports."""

from __future__ import annotations


def render_markdown(report: dict) -> str:
    lines = [
        "# NPU Micro-task Companion Report",
        "",
        f"- Passed: `{report['passed']}`",
        f"- Mode: `{report['mode']}`",
        f"- Provider stage: `{report.get('provider_stage')}`",
        f"- Provider work verified: `{report.get('provider_work_verified')}`",
        f"- Provider role counted: `{report.get('provider_role_counted')}`",
        f"- Role rejection reason: `{report.get('role_rejection_reason')}`",
        f"- Provider backend: `{report.get('provider_backend')}`",
        f"- Provider compute device: `{report.get('provider_compute_device')}`",
        f"- Provider device verified: `{report.get('provider_device_verified')}`",
        f"- Device detected: `{report.get('device_detected')}`",
        f"- Model loaded: `{report.get('model_loaded')}`",
        f"- Health check passed: `{report.get('health_check_passed')}`",
        f"- Workload performed: `{report.get('workload_performed')}`",
        f"- Useful output produced: `{report.get('useful_output_produced')}`",
        f"- CPU provider fallback performed: `{report.get('cpu_provider_fallback_performed')}`",
        f"- NPU peer activity requested: `{report['npu_peer_activity_requested']}`",
        f"- NPU peer activity performed: `{report['npu_peer_activity_performed']}`",
        f"- NPU device execution performed: `{report['npu_device_execution_performed']}`",
        f"- NPU provider execution performed: `{report['npu_provider_execution_performed']}`",
        f"- NPU peer evidence verified: `{report.get('npu_peer_evidence_verified')}`",
        f"- NPU response schema valid: `{report.get('npu_response_schema_valid')}`",
        f"- NPU native tool-loop required: `{report.get('npu_native_tool_loop_required')}`",
        f"- NPU native tool-loop error: `{report.get('npu_native_tool_loop_error')}`",
        f"- NPU peer follow-up required: `{report.get('npu_peer_followup_required')}`",
        f"- NPU micro task kind: `{report.get('npu_micro_task_kind')}`",
        f"- NPU micro decision: `{report.get('npu_micro_decision')}`",
        f"- NPU micro task closed: `{report.get('npu_micro_task_closed')}`",
        f"- NPU micro provider required: `{report['npu_micro_provider_required']}`",
        f"- NPU micro provider performed: `{report['npu_micro_provider_execution_performed']}`",
        f"- NPU micro model loaded: `{report['npu_micro_provider_model_loaded']}`",
        f"- NPU micro classification: `{report['npu_micro_provider_classification']}`",
        f"- Leader packet consumed: `{report['leader_packet_consumed']}`",
        f"- Leader packet: `{report['leader_packet']}`",
        f"- Leader packet heap universe contract: `{report['leader_packet_heap_universe_contract']}`",
        f"- Leader packet pointer contract: `{report['leader_packet_pointer_contract']}`",
        f"- Leader packet time counter: `{report['leader_packet_time_counter_contract']}`",
        f"- Leader packet startup artifacts: `{report['leader_packet_startup_artifacts_count']}`",
        f"- Leader packet broker tool evidence: `{report['leader_packet_broker_tool_evidence_count']}`",
        f"- NPU OpenVINO native tool loop supported: `{report['native_tool_loop_supported']}`",
        f"- NPU OpenVINO native tool calls: `{report['native_tool_call_count']}`",
        f"- NPU device workload requested: `{report['npu_device_workload_requested']}`",
        f"- NPU device workload performed: `{report['npu_device_workload_performed']}`",
        f"- NPU device workload: `{report['npu_device_workload']}`",
        f"- NPU activity classification: `{report['npu_activity_classification']}`",
        f"- Request input chars: `{report.get('request_input_chars')}`",
        f"- Request input sha256: `{report.get('request_input_sha256')}`",
        f"- Request input omitted: `{report.get('request_input_omitted')}`",
        f"- Request input ref: `{(report.get('request_input_ref') or {}).get('path') or ''}`",
        f"- Request input preview: `{report.get('request_input_preview') or ''}`",
        f"- Response text ref: `{(report.get('response_text_ref') or {}).get('path') or ''}`",
        f"- Response text chars: `{report.get('response_text_chars')}`",
        f"- Response text sha256: `{report.get('response_text_sha256')}`",
        f"- Response text tail: {report.get('response_text_tail') or ''}",
        f"- NPU activity limit: {report['npu_activity_limit']}",
        f"- Legacy NPU auditor used: `{report['guardrails']['legacy_npu_auditor_used']}`",
        f"- Provider execution performed: `{report['guardrails']['provider_execution_performed']}`",
        "",
        "## Recommendation",
        "",
        report["recommendations"][0].get("summary_tail") or "",
    ]
    return "\n".join(lines) + "\n"
