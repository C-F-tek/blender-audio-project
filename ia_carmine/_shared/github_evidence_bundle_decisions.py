...
from ia_carmine._shared.github_evidence_bundle_io import as_list, list_contains
from ia_carmine._shared.github_evidence_bundle_reports import report_summaries
...
def selected_chunks_evidence_seen(
    selected_chunks_evidence: list[dict[str, Any]],
) -> bool:
    """Return whether at least one compact selected-chunks report is present."""
    return any(
        item.get("exists") is True and item.get("json_ok") is True
        for item in selected_chunks_evidence
    )
...
def selected_chunks_budget_respected(
    selected_chunks_evidence: list[dict[str, Any]],
) -> bool:
    ...
    for summary in report_summaries(selected_chunks_evidence):
        ...
        if (
            ...
            and isinstance(max_total_chars, int)
...
def context_excludes_npu(context: dict[str, Any]) -> bool:
    ...
    if NPU_REAL_WORKLOAD_REPORT in excluded_context:
        return True
    advisory_routing = (
        context.get("advisory_context_routing")
        if isinstance(context.get("advisory_context_routing"), dict)
        else {}
    )
