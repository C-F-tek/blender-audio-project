...
from ia_carmine._shared.openvino_model_discovery import (
...
def ollama_tool_call_selection_prompt(prompt: str, provider_delta: str) -> str:
    ...
    decision_rule = (
        "Keep heap proposal text as primary. GPU1 may drive operative broker requests and owns final synthesis. GPU0 has the same Ollama tool-call schema but its results are peer-only refinement/veto/evidence that require a later GPU1 consumption turn. NPU tool calls are diagnostic/veto evidence only. For code product, matrix, lab, patch synthesis, runtime refs or memory gaps, choose one broker tool through message.tool_calls. Use generic_write when the current lane cannot yet produce code and needs a refined request/next-turn plan; if no tool is called, useful prose may still be captured as generic_write evidence. "
...
def openvino_tool_loop_report(
    *,
    ...
    if allow_discovery:
        model_dir, model_dir_source = discover_openvino_tool_model_dir(
            repo_root, device=device, explicit_model_dir=model_dir
        )
    else:
        model_dir = (
    ...
    try:
        ...
        completed = subprocess.run(
            [
                str(runner),
                "-m",
                "ia_carmine._shared.provider_tool_loop",
        ...
        report = dict(base)
        report["native_tool_decision"] = "python_missing_or_unlaunchable"
        report["classification"] = "openvino_tool_loop_python_missing"
        report["errors"].append(f"OpenVINO tool-loop child could not start: {type(exc).__name__}: {exc}")
        report["elapsed_sec"] = round(time.perf_counter() - started, 4)
        return report
