...
try:
    from ia_carmine.runtime.runtime_tool.file_refs import RuntimeFileRefResolver
    ...
    from ia_carmine.runtime.runtime_tool.file_refs import RuntimeFileRefResolver  # type: ignore
...
def render_source_allowlist_contract(candidates: list[str]) -> str:
    lines = [
        ...
        "- For refactor/OOB tasks, use existing allowed source paths first; if the operator names a non-existent package but the repo has an equivalent package in this allowlist, use the allowlisted existing package.",
        "- New file proposals are valid only when runtime file refs/tool evidence show no existing module can satisfy the responsibility.",
...
def source_anchor_feedback(candidates: list[str], requested: list[Any] | None = None) -> str:
    ...
    lines = [
        ...
        "If a request names a non-existent package and an equivalent existing package is allowlisted, use the allowlisted existing package instead of proposing new files.",
